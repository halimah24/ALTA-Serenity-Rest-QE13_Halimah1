Feature: Serenity Rest QE 13
  ...
  Scenario: Post create user with valid json
    ...
    And Response body name was "halimah" and job was "QA Engineer"
    And Validate get list users json schema "CreateUserSchema.json"
   @Latihan
   Scenario Outline: Update user with valid id and json
     ...
     Examples:
       | id | json             | name           | job                |
       | 1  | UpdateUser.json  | halimah Edit   | QA Engineer Edit   |
       | 2  | UpdateUser2.json | halimah Edit 2 | QA Engineer Edit 2 |
